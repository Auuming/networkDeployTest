...
function ChatRoom({ chat, messages, onSendMessage, currentClientName, socket, currentSocketId }: ChatRoomProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900">
      ...
      <form onSubmit={handleSubmit} className="p-5 border-t border-gray-800 flex gap-3 bg-gray-900">
        <input
          type="text"
          value={inputMessage}
          onChange={(e) => setInputMessage(e.target.value)}
          placeholder={`Type a message${chat.type === 'private' ? ' (private)' : ' (group)'}...`}
        ...
        <button 
          ...
          style={{ background: !inputMessage.trim() ? '#374151' : 'linear-gradient(to right, #473472, #53629E)' }}
