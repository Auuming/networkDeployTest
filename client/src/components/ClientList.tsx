...
function ClientList({
  ...
  return (
    <div className="p-5 border-b border-gray-800 flex-shrink-0 max-h-[50%] overflow-y-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-base md:text-lg font-semibold text-gray-200">
          Online Users
        </h2>
        <span className="bg-primary text-white px-2.5 py-1 rounded-xl text-xs font-semibold">
          {clients.length}
        </span>
      </div>
      <div className="flex flex-col gap-2">
        {clients.length === 0 ? (
          ...
          clients.map((client) => (
            <div
              key={client.socketId}
              className={`flex items-center gap-3 p-2.5 md:p-3 rounded-lg cursor-pointer transition-colors ${
                client.socketId === currentClientId
                  ? "bg-[#D6F4ED] cursor-default border border-[#87BAC3] text-gray-900"
                  : "hover:bg-[#D6F4ED]/30 hover:border-[#87BAC3]/50"
              }`}
              onClick={() =>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  {client.name}
                  {client.socketId === currentClientId && (
                    <span className="text-[#53629E] text-xs md:text-sm font-semibold ml-1">
                      {" "}
                      (You)
                    </span>
                  )}
